...
export class GraficoComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    if (this.x3.length > 0 && this.y3.length > 0){
      this.contexto.stroke();
      //Constroi o grafico
      this.contexto.strokeStyle = "darkgreen";
      this.contexto.lineWidth = 2;
      this.contexto.beginPath(); // Reset path for the axes
      this.contexto.moveTo(this.LGX(this.x3[0]), this.LGY(this.y3[0]));
      for (let i = 1; i < this.x3.length; i++) {
        let x = this.LGX(this.x3[i])
        let y = this.LGY(this.y3[i]);
        this.contexto.lineTo(x, y);
        this.contexto.stroke();
      }
    }
